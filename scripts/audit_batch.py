...
运行逻辑：
  ...
  3. --skip（默认）跳过日志中 status=success 的图片；--no-skip 忽略日志全部重跑
  4. 并发处理缺失图片，每完成一张更新日志（合并新旧记录，覆写排序）

用法:
  # 默认：跳过已有成功记录
  python scripts/audit_batch.py --dataset cierra --start-from 1
  
  # 跳过 + 指定范围
  python scripts/audit_batch.py --dataset cierra --start-from 101 --limit 100
  
  # 强制重跑（忽略日志和已有输出）
  python scripts/audit_batch.py --dataset cierra --no-skip --start-from 1
  
  # 强制重跑指定范围
  python scripts/audit_batch.py --dataset cierra --no-skip --start-from 50 --limit 10
"""
...
def _extract_clean_tags(content: str, reasoning: str) -> str:
...
def process_one(image_path: Path, output_dir: Path, system_prompt: str, api_key: str) -> dict:
    """处理单张图片，返回结果字典"""
    ...
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:
            ...
            answer = _extract_clean_tags(content, reasoning)
            if not answer:
...
def main():
    ...
    if args.skip and log_entries:
        ...
        to_process = [f for f in to_process
                      ...
                      or log_entries[get_image_name(f)]['status'] != 'success'
    ...
    if args.skip:
        print(f"跳过模式: 开 ({skipped_count} 张已有成功记录)")
    else:
        print(f"跳过模式: 关 (全部重跑)")
    if to_process:
    ...
    def process_and_report(img_path):
        ...
        r = process_one(img_path, output_dir, system_prompt, api_key)
        elapsed = time.time() - t0
        icon = '✅' if r['status'] == 'success' else '❌' if r['status'] == 'failed' else '⏭️'
        with print_lock:
            print(f"  {icon} {r['image']:15s} {r['original_count']:2d}→{r['new_count']:2d}  ({elapsed:.0f}s)  {r['error'] if r['error'] else ''}")
