...
- 删除多余的人数标签（如 `multiple_girls`、`multiple_boys`、所有与人数的实际数字不符的高位数字标签）。
...
### 2.3 作品/系列名
删除任何作品或系列相关标签。
例如：touhou、vocaloid、fate、genshin_impact、blue_archive、grimoire_of_alice 等。
...
- **画面类型**：illustration、sketch、lineart、painting

注意：不要每张图都重复堆叠风格词。让 `@cierra-rabit` 负责绑定画风。
...
- **明显的画面类型**：sketch、lineart、flat_color、painting、watercolor、grayscale
...
**示例：**
- 图片背景有明显夜景和月亮 → 补充 night、moon
- 画面中角色明显在跳舞 → 补充 dancing
- 图片是线稿风格 → 补充 lineart、sketch

---

## 5. 标签上限

最终输出控制在 **15~20 个标签**（不含触发词 `@cierra-rabit`）。
...
只输出修正后的标签列表，用逗号分隔。第一项必须是 `@cierra-rabit`。

```
@cierra-rabit, solo, upper_body, looking_at_viewer, dress, smile, blonde_hair, blue_eyes, flower, sitting, indoors, window_light
```
