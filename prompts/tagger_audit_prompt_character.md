...
当训练图片中出现超过 1 个人物时，需要根据互动程度决定是否保留该图，以及如何打标签。

### 核心原则

**绝不在有他人出现的图上使用 `solo` 标签。** `solo` 只用于严格单人图。即使另一人只露出局部（手、手臂、身体边缘，没有脸），也不使用 `solo`。

### 分类标准

| 情况 | 是否保留 | 标签策略 |
|---|---|---|
| 只有另一人的手/手臂/身体边缘入镜，没有脸，轻微接触目标角色 | 保留 | 使用实际人数标签（如 `1girl`），同时保留动作标签如 `holding`、`hand_on_shoulder`、`reaching_out`、`grabbing` 等。删除 `solo`。 |
| 另一人的手遮挡目标角色脸/胸口/重要服装特征 | 建议删除 | 会污染角色特征学习。如果确实要保留，标签中加 `holding`、`hand_on_face` 等，且该图不应超过总量 5%。 |
| 两人有明显互动（牵手、拥抱、贴脸、贴身、遮挡主体） | 谨慎保留 | 保留时标签精准描述互动：`hug`、`holding_hands`、`embrace`、`leaning_forward`。删除 `solo`。此类图总量控制在 5%~10%。 |
| 目标角色被抱住/身体大量重叠/互动姿势复杂 | 建议删除 | 最容易学出多手、多肢体、多人残影。 |
| 目标角色表情/姿势因互动而非常特殊 | 少量可放 | 仅作为表情/情绪补充，不要多。 |

### 标签要点

- 人数标签优先按实际人数标注（`1girl`、`2girls`、`1girl 1boy` 等）。
- 局部他人的图：保留目标角色的完整特征标签（瞳色、发色、发型、服装、表情、姿势），额外添加动作标签反映互动。
- 不要为了掩盖他人存在而删除相关动作标签——让模型知道这是"有人在场"的图，而不是 solo。
